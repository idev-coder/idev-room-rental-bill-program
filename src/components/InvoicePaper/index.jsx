...
import { Paper, styled } from "@idev-coder/idev-ui";
...
export default function InvoicePaper(props) {
  ...
  React.useEffect(() => {
              ...
              const newTable = data.table.map((list, key) => {
                ...
                if (list.no === "2") {
                  return {
                    ...
                    description: {
                      ...list.description,
                      unitBefor: invoices[0].table.find(
                        ({ no }) => no === list.no
                      ).description.unitAffter,
                    },
                  };
                }
                ...
                if (list.no === "3") {
                  setWaterUnitPrice(
                    invoices[0].table.find(({ no }) => no === list.no).unit
                  );
                  return {
                    ...list,
                    description: {
                  ...
                  const t = newTable.filter((list) => {
                    if (list.no === "2") {
                      if (!list.unit) {
                        ...
                        setElectricUnitPrice(
                          invoices[0].table.find(({ no }) => no === list.no)
                            .unit
                        );
                        return list;
                      }
    ...
    if (props.mode === "updated" || props.mode === "view") {
      if (isElectron()) {
        ...
        axios
          .post("/invoices/findByPk", { id: props.row.id })
          .then(({ data: invoice }) => {
            if (invoice) {
              axios
                .post("/units/findByPk", { id: "1" })
                .then(({ data: unit }) => {
  ...
  return (
    <React.Fragment>
      ...
      {(props.mode === "created" || props.mode === "updated") && (
        <React.Fragment>
          <div
            style={{
              position: "fixed",
                    ...
                    <InputBase
                      ...
                      onChange={(event) => {
                        ...
                        const newTable = data.table.map((v) => {
                          if (v.no === "2") {
                            let unit =
                              parseFloat(v.description.unitAffter) -
                              parseFloat(v.description.unitBefor);
      ...
      {props.mode === "created" && (
        <ButtonSave
          onClick={() => {
            if (isElectron()) {
              window.api.invoice
                .create({ body: { ...data, total: total(data.table) } })
                .then((invoice) => {
                  if (invoice) {
                    setData({
                      id: randomId(),
                    ...
                    window.api.unit
                      .update({
                        body: {
                          eUnit: electricUnitPrice,
                          wUnit: waterUnitPrice,
                        },
                        options: {
                          where: {
                            id: "1",
              ...
              axios
                .post("/invoices/create", {
                  body: { ...data, total: total(data.table) },
                })
                .then(({ data: invoice }) => {
                  if (invoice) {
                    setData({
